// _shared/lib/scoring.js — ported from server/scoring.js.
// CHANGES:
//   - Replaced readFileSync/writeFileSync file loading with JSON import attribute
//     for the bundled scoring-config seed default.
//   - Replaced process.env with getEnv (no process.env usage in original scoring.js
//     anyway — this is a pre-emptive port of the import style).
//   - saveScoringConfig() is removed — persisting config is a DB concern, not a
//     file-system concern in edge functions.
//   - loadScoringConfig() is refactored to:
//       loadScoringConfig(row?) — takes the app_config jsonb value (or null) and
//                                  merges over the bundled seed defaults, fail-soft.
//     The old {fresh:true} option is removed (no file cache to invalidate).
//   - _objectionCats / objectionCategories() now loads from JSON import attribute
//     instead of readFileSync.
//   - chat/extractJson/DETERMINISTIC_SAMPLING imports come from "./llm.js".
//
// SIGNATURE CHANGES (document for function-builder agents):
//   OLD: loadScoringConfig({fresh?}) → config (reads file)
//        saveScoringConfig(next)      → persists to disk
//   NEW: loadScoringConfig(row?)      → pure function; edge fn passes DB row value
//        saveScoringConfig            → REMOVED (use DB upsert instead)

import objectionsRaw from "../seed/objections.json" with { type: "json" };
import scoringConfigSeed from "../seed/scoring-config.json" with { type: "json" };
import { chat, extractJson, DETERMINISTIC_SAMPLING } from "./llm.js";

// ---------------------------------------------------------------------------
// Objection categories (loaded from seed)
// ---------------------------------------------------------------------------
...
/**
 * loadScoringConfig(row?)
 *
 * Takes the app_config jsonb value (or null) and merges over the bundled
 * seed defaults, fail-soft. Called by edge functions after fetching the
 * app_config row from the DB.
 *
 * SIGNATURE CHANGE from server/scoring.js:
 *   OLD: loadScoringConfig({fresh?}) — reads disk, caches
 *   NEW: loadScoringConfig(row?)     — pure function; edge fn passes DB row value
 *
 * @param {object|null} row  The app_config.value jsonb or null.
 * @returns {object}  Merged scoring config.
 */
export function loadScoringConfig(row) {
  // Merge: hardcoded defaults <- seed JSON file <- live DB row
  let base;
  try {
    base = coerceConfig(scoringConfigSeed);
  } catch {
    base = DEFAULT_SCORING_CONFIG;
  }
  if (!row) return base;
  try {
    // coerceConfig fills in missing keys from base; spread row over base to get
    // the full merged object.
    return coerceConfig({ ...base, ...row });
  } catch {
    return base;
  }
}
...
export function isBackchannel(text, config) {
  ...
  const set = new Set((cfg.backchannelWords || []).flatMap(tokens).filter(Boolean));
...
function buildScoringPrompt({ message, recentTurns = [], phase = 1, turnType, courseName, openObjections = [], payAskCountBefore } = {}, config) {
  ...
  const absenceRule = cfg.neverPenalizeAbsence
...
export function scoringPromptForInspection(opts = {}) {
  return buildScoringPrompt(opts, loadScoringConfig(null));
}
...
export async function scoreMessage(message, opts, legacyCourseName, chatOpts = {}, scoringCfg) {
  const config = scoringCfg || loadScoringConfig(null);
