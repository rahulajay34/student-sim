// _shared/lib/promptConfig.js — ported from server/promptConfig.js.
// CHANGES:
//   - Replaced readFileSync/statSync file loading with JSON import attribute for the
//     bundled seed default.
//   - Replaced getPromptConfig() / invalidatePromptConfigCache() with:
//       loadPromptConfig(row?)  — takes the app_config jsonb value (or null) and
//                                  merges it over the bundled seed defaults, fail-soft.
//       getPromptConfig()       — LEGACY alias: returns DEFAULT_PROMPT_CONFIG (no DB
//                                  access; kept so server/prompt.js imports keep working).
//   - Removed process.env usage — not present in original anyway.
//   - NO direct DB access inside this module; the edge functions fetch the app_config
//     row and pass it in.
//
// SIGNATURE CHANGES (document for function-builder agents):
//   OLD: getPromptConfig() → merged config (reads file)
//   NEW: loadPromptConfig(row?)  → merged config (takes DB row value or null)
//        getPromptConfig()        → DEFAULT_PROMPT_CONFIG (no merging; kept for legacy callers)
//        invalidatePromptConfigCache() → no-op (kept for legacy import compat)

import promptConfigSeed from "../seed/prompt-config.json" with { type: "json" };
...
// Module-level active config (set by edge functions via setActivePromptConfig).
let _activeRow = undefined;
let _activeConfig = undefined;

/**
 * loadPromptConfig(row?)
 *
 * Takes the app_config jsonb value (or null) and merges it over the bundled
 * seed defaults, fail-soft. Called by edge functions after fetching the
 * app_config row from the DB:
 *
 *   const { data } = await supabase.from("app_config")
 *     .select("value").eq("key", "prompt_config").single();
 *   const cfg = loadPromptConfig(data?.value);
 *
 * SIGNATURE CHANGE from server/promptConfig.js:
 *   OLD: getPromptConfig() — reads disk, caches by mtime
 *   NEW: loadPromptConfig(row?) — pure function; the edge fn passes the DB row value
 *
 * @param {object|null} row  The app_config.value jsonb or null.
 * @returns {object}  Merged prompt config.
 */
export function loadPromptConfig(row) {
  // Try to merge the seed file first (over the hardcoded defaults), then the
  // live DB row on top. This three-way merge means the seed file acts as a
  // "source of truth" fallback that is always fresher than the hardcoded
  // DEFAULT_PROMPT_CONFIG constant (which may lag after a seed update).
  let base;
  try {
    base = mergeConfig(DEFAULT_PROMPT_CONFIG, promptConfigSeed);
  } catch {
    base = DEFAULT_PROMPT_CONFIG;
  }
  if (!row) return base;
  try {
    return mergeConfig(base, row);
  } catch {
    return base;
  }
}
 ...
 * getPromptConfig()
 *
 * Returns the active config if setActivePromptConfig has been called (edge
 * functions call this before any prompt composition path). Falls back to the
 * seed-merged config (same as loadPromptConfig(null)) for legacy callers that
 * do not drive the active-row lifecycle.
 */
export function getPromptConfig() {
  if (_activeConfig !== undefined) return _activeConfig;
  return loadPromptConfig(null);
}

/**
 * invalidatePromptConfigCache()
 * No-op kept for legacy import compatibility. Config is not cached in this module.
 */
...
/**
 * setActivePromptConfig(rowValueOrNull)
 *
 * Called by edge functions after fetching the live app_config 'prompts' row.
 * Updates the module-level active row; recomputes and caches the merged config.
 * getPromptConfig() will return this merged config until the next set call.
 *
 * @param {object|null} rowValue  The app_config.value jsonb or null.
 */
export function setActivePromptConfig(rowValue) {
  _activeRow = rowValue ?? null;
  _activeConfig = loadPromptConfig(_activeRow);
}
