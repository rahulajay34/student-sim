...
Report    { id, sessionId, assignmentId|null, counsellorId, counsellorName, personaName, scenarioTitle,
            ...
            newReport: { total: number,   // 0-100; = (sum of 8 parameter scores / 40) * 100
                         parameters: [{ key, label, score: 0..5, summary }] } | null,
            //   Admin/superadmin-only 8-parameter additive re-scoring (separate from the rubric system).
            //   Fixed parameter order: rapport_opening, needs_discovery, programme_presentation,
            //   objection_handling, product_knowledge, closing_payment_ask, communication_empathy,
            //   personalised_experience. Generated by report-worker Call F (mode:"reasoning").
            //   Stripped for non-admins at edge: deleted from GET /reports and GET /reports/:id responses
            //   (same boundary as integrityCheck). null / absent on old reports or while generating.
            //   Migration 0010_new_report.sql adds reports.new_report (jsonb) + extends commit_report.
            //   Rendered in ReportDetail as "New Report Section" beneath an "Old Report Section" divider.
            generatedAt }
            // Report generation: LLM calls A (rubric + phaseBreakdown), B (narrative + keyMoments), C (drills), D (personaAddressed), E (integrityCheck), F (newReport)
            //   run concurrently via Promise.allSettled. Fail-soft: A fail → fallback; B/C/D/E/F fail → partial:true + section defaults.
...
- `0009_integrity_check.sql` — adds `reports.integrity_check` (jsonb); extends `commit_report` RPC to
  accept `integrity_check` in the patch; seeds the 24 default integrity probes into `app_config` key
...
| GET | `/integrity-probes` | — | **admin-only** — `{ probes:[{id,category,question,groundTruth,active}], guidelines:[] }` merged over 24 seeded defaults; stored in `app_config` key `integrityProbes`. 403 for counsellors. |
